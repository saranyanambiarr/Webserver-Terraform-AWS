...
connection{
    type = "ssh"
    user = "ec2-user"
    private_key = file("C:/Users/Saranya/Desktop/Terraform/Task1keysar.pem")
...
resource "aws_s3_bucket" "task1bucketsaranya00saranya00" {
  bucket = "task1bucketsaranya00saranya00"
  acl    = "private"
  tags = {
  Name = "task1bucketsaranya00saranya00"
 }
}
...
resource "aws_s3_bucket_public_access_block" "S3PublicAccess" {
  bucket = "${aws_s3_bucket.task1bucketsaranya00saranya00.id}"
...
resource "aws_s3_bucket_object" "bucketObject" {
   for_each = fileset("C:/Users/Saranya/Desktop/Musings/images", "**/*.jpg")
   bucket = "${aws_s3_bucket.task1bucketsaranya00saranya00.bucket}"
   key = each.value
   source = "C:/Users/Saranya/Desktop/Musings/images/${each.value}"
...
resource "aws_cloudfront_origin_access_identity" "origin_access_identity" {
 comment = "task1bucketsaranya00saranya00"
}

resource "aws_cloudfront_distribution" "Task1CF" {
origin {
 domain_name = "${aws_s3_bucket.task1bucketsaranya00saranya00.bucket_regional_domain_name}"
...
depends_on=[
 aws_s3_bucket.task1bucketsaranya00saranya00
]
}
...
resources = ["${aws_s3_bucket.task1bucketsaranya00saranya00.arn}/*"]
principals {
type        = "AWS"
